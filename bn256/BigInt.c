...
void gfp_from_bignum(gfp_t c, struct bn* n) {
    c[0] = n->array[0];
    c[0] |= ((uint64_t)(n->array[1])) << 32;
    c[1] = n->array[2];
    c[1] |= ((uint64_t)(n->array[3])) << 32;
    c[2] = n->array[4];
    c[2] |= ((uint64_t)(n->array[5])) << 32;
    c[3] = n->array[6];
...
int bignum_wordlen(struct bn* n) {
    ...
    while (i >= 0 && n->array[i] == 0) i--;
    // FIXME: what if the first word is a zero? Go in reverse instead.
